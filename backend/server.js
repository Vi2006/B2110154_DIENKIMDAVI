  ...
  mongoClient.connect(
    ...
    async function (error, client) {
      ...
      app.post("/getUser", userAuth, async function (request, result) {
        ...
        result.json({
          status: "Thành công",
          message: "Dữ liệu đã được tìm nạp.",
          user: user,
        });
      });
      ...
      app.post("/registration", async function (request, result) {
        ...
        if (!name || !email || !password) {
          result.json({
            status: "Lỗi",
            message: "Vui lòng nhập tất cả các giá trị.",
          });
          return;
        }
        ...
        if (email == global.adminEmail) {
          result.json({
            status: "Lỗi",
            message:
              "Rất tiếc, bạn không thể tạo tài khoản bằng địa chỉ email này.",
          });
          return;
        }
        ...
        bcryptjs.genSalt(10, function (error, salt) {
          bcryptjs.hash(password, salt, async function (error, hash) {
            ...
            result.json({
              status: "success",
              message: "Tài khoản đã được tạo. Hãy đăng nhập ngay bây giờ.",
            });
          });
        });
      });
      ...
      app.post("/cashOnDelivery", async function (request, result) {
        ...
        result.json({
          status: "success",
          message:
            "Đã nhận được đơn đặt hàng của bạn. Chúng tôi sẽ cho bạn biết qua E-mail về việc theo dõi đơn hàng của bạn.",
        });
      });
